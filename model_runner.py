...
class ModelRunner:
    ...
    def allocate_kv_cache(self):
        ...
        if num_gpu_blocks <= 0:
            raise RuntimeError("Not enough GPU memory for KV cache.")
        config.num_kvcache_blocks = num_gpu_blocks

        if config.num_cpu_kvcache_blocks >= 0:
            num_cpu_blocks = config.num_cpu_kvcache_blocks
        else:
            cpu_bytes_per_rank = int(config.cpu_kvcache_gb * 1024**3 / self.world_size)
            num_cpu_blocks = cpu_bytes_per_rank // block_bytes

        if config.cpu_kvcache_gb > 0 and num_cpu_blocks == 0:
            raise RuntimeError("cpu_kvcache_gb is smaller than one logical KV block.")
        config.num_cpu_kvcache_blocks = num_cpu_blocks

        self.kv_cache = torch.empty(2, hf_config.num_hidden_layers, num_gpu_blocks, self.block_size, num_kv_heads, head_dim, dtype=self.model_dtype, device="cuda")

        self.cpu_kv_cache = None
        if num_cpu_blocks > 0:
            self.cpu_kv_cache = torch.empty(2, hf_config.num_hidden_layers, num_cpu_blocks, self.block_size, num_kv_heads, head_dim, dtype=self.model_dtype, device="cpu", pin_memory=True)
